...
class xc_Player(
        ...
        try:
            if globalsxp.STREAMS.trial != '':
                ...
                try:
                    ...
                    # Make sure the path ends with a slash for join to work
                    # properly
                    path_movies = globalsxp.Path_Movies
                    if not path_movies.endswith('/'):
                        path_movies += '/'

                    self.timeshift_url = join(path_movies, self.filename)
    ...
    def createMetaFile(self, filename, filmtitle):
        try:
            ...
            meta_path = join(globalsxp.Path_Movies, filename + '.meta')
            with open(meta_path, 'w') as f:
                f.write('%s\n%s\n%s\n%i\n' %
                        (serviceref.toString(), filmtitle, "", int(time())))
        except Exception as e:
...
class xc_Play(Screen):
    ...
    def runList(self):
        ...
        if path.lower().endswith('.m3u'):
            self.session.open(xc_M3uPlayx, path)
        else:
    ...
    def downloadz(self):
        if self.downloading:
            ...
            self.session.open(
                ...
                self.in_tmp,
                self.urlm3u)
            self.downloading = False  # Set after opening the screen
            self.refreshmylist()
    ...
    def downloadProgress(self, recvbytes, totalbytes):
        ...
        if totalbytes > 0:
            ...
            self['progresstext'].text = '{} of {} kBytes ({:.2f}%)'.format(
                recvbytes // 1024, totalbytes // 1024, percent)
        else:
    ...
    def check(self, result):
        if file_exists(self.in_tmp):
            ...
            self.showError('File missing after download')
        ...
        try:
            with open(self.name, "r", encoding="utf-8", errors="ignore") as f:
                ...
                for line in f:
                    ...
                    # EXTINF line (channel info) - compatible with all formats
                    if line.startswith("#EXTINF:"):

                        parts = line.split(",", 1)
                        if len(parts) > 1:
                            desk_tmp = parts[1].strip()

                        # Extract group if present
                        group_match = search(r'group-title="([^"]+)"', line)
                        if group_match:
                            group = group_match.group(1)
                            desk_tmp = "[" + group + "] " + desk_tmp

                    # URL line (stream)
                    elif line and line.startswith("http"):
                        self.urls.append(line)
                        self.names.append(desk_tmp)
                        desk_tmp = ""

                    # Support for XML-like formats (without changing original
                    # logic)
                    elif "<stream_url><![CDATA" in line:
                        stream_url = line.split(
            ...
            print("Error reading M3U file: " + str(e))
            self.session.open(
                MessageBox,
                "Error reading M3U file: " +
                str(e),
        ...
        if idx is None or idx < 0 or idx >= len(self.names):
            ...
            if ext in EXTDOWN or ext == '.avi':
                if answer is None:
                    ...
                    self.in_tmp = join(globalsxp.Path_Movies, filename.lower())
                    if file_exists(self.in_tmp):
                        try:
                            remove(self.in_tmp)
                        except Exception as e:
                    ...
                    if cfg.pdownmovie.value == "Direct":
                        ...
                        if "https" in str(self.urlm3u):
                            cmd = "wget --no-check-certificate --no-cache --no-dns-cache -U %s -c '%s' -O '%s%s'" % (
                                'Enigma2 - XC Forever Plugin', self.urlm3u, str(globalsxp.Path_Movies), self.filename)
                        self.timeshift_url = self.in_tmp
                        JobManager.AddJob(
                            downloadJob(
                                self,
                                cmd,
